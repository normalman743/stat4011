...
def get_A_B_only(bad_f1, predicted_bad_count, layer):
    ...
    baseline
    # print(f" A_TP: {result['TP']}")
    # print(f" A_FP: {result['FP']}")
    # print(f" A_FN: {result['FN']}")
    # print(f" A_TN: {result['TN']}")

    # print(f"realgood: {realgood}")
    # print(f"realbad: {realbad}")
    if result:
        if baseline['predict']:
            # print(f"B_TP : realgood {realgood} - result['FP'] {result['FP']}")  # 修正了这里的格式
            # print(f"B_FP : realbad {realbad} - result['TP'] {result['TP']}")  # 修正了这里的格式
            return {
                'A_TP': result['FP'],
                'A_FP': result['TP'],
                'B_TP': realgood - result['FP'],
                'B_FP': realbad - result['TP'],
            }
        else:
