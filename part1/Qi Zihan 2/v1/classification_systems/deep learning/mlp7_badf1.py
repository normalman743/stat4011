...
def find_optimal_threshold(y_true, y_prob, metric='bad_f1', thresholds=None):
    """在验证集上寻找最优阈值"""
...
def train_model_with_threshold_tuning(model, train_loader, val_loader, config, class_weights, epochs=100):
    """训练模型并进行阈值调优"""
    ...
    for epoch in range(epochs):
        ...
        optimal_bad_f1 = bad_f1
        
        if epoch % 10 == 0 or epoch >= epochs - 5:
            optimal_threshold, optimal_score, _ = find_optimal_threshold(
                val_targets, val_probabilities, metric='bad_f1'
            )
            optimal_bad_f1 = optimal_score
        
        # 存储本轮指标
        epoch_metrics.append({
            'epoch': epoch + 1,
            'train_loss': train_loss / len(train_loader),
            'bad_f1': bad_f1,  # 默认阈值0.5的Bad F1
            'optimal_threshold': optimal_threshold,
            'optimal_bad_f1': optimal_bad_f1,  # 优化阈值的Bad F1
            'f1_macro': val_f1_macro,
            'f1_weighted': val_f1_weighted,
            'accuracy': val_accuracy,
            'precision_weighted': val_precision_weighted,
            ...
            'bad_precision': bad_precision,
            'bad_recall': bad_recall,
            'val_probabilities': val_probabilities  # 保存概率用于后续阈值调优
        })
        
        # 使用优化后的Bad F1进行学习率调度和早停
        scheduler.step(optimal_bad_f1)
        
        # 每10轮或最后几轮打印详细指标
        if epoch % 10 == 0 or epoch >= epochs - 3:
            print(f'Epoch [{epoch+1:3d}]: Bad_F1={bad_f1:.4f} -> 优化后={optimal_bad_f1:.4f} (阈值={optimal_threshold:.3f})')
            print(f'              mF1={val_f1_macro:.4f}, F1_w={val_f1_weighted:.4f}, Acc={val_accuracy:.4f}')
            print(f'              Good: Prec={good_precision:.4f}, Recall={good_recall:.4f}, F1={good_f1:.4f}')
            print(f'              Bad:  Prec={bad_precision:.4f}, Recall={bad_recall:.4f}, F1={bad_f1:.4f}')
        
        if early_stopping(optimal_bad_f1, model):  # 使用优化后的Bad F1进行早停
            print(f'Early stopping at epoch {epoch+1}')
            break
    
    return epoch_metrics

def predict_with_optimal_threshold(model, data_loader, threshold=0.5):
    """使用最优阈值进行预测"""
    model.eval()
    predictions = []
    probabilities = []
...
def main_best_config_cv_with_threshold():
    """主函数 - 加入阈值调优的CV分析"""
    
    print("=" * 80)
    print("🎯 MLP最佳配置CV预测分析 - Bad客户F1指标版 + 阈值调优")
    print("=" * 80)
    ...
    best_config = {
        ...
        'lr': 0.001, 
        'smoothing': 0.03, 
        'name': '保守改进_BadF1_ThresholdTuned'
    }
    
    print(f"使用配置: {best_config}")
    print("主要评估指标: Bad客户F1分数 + 阈值调优")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X_train_scaled, y_train)):
        ...
        # 训练模型（包含阈值调优）
        model = RefinedMLP(input_dim=X_train_scaled.shape[1]).to(device)
        epoch_metrics = train_model_with_threshold_tuning(
            model, train_loader, val_loader, best_config, class_weights, epochs=100
        )
        
        # 获取最佳轮次的指标 - 使用优化后的Bad F1
        best_epoch_idx = np.argmax([m['optimal_bad_f1'] for m in epoch_metrics])
        best_metrics = epoch_metrics[best_epoch_idx]
        optimal_threshold = best_metrics['optimal_threshold']
        optimal_thresholds.append(optimal_threshold)
        
        print(f"🎯 Fold {fold_idx+1} 最优阈值: {optimal_threshold:.3f}")
        
        # 验证集预测(OOF) - 使用最优阈值
        val_preds, val_probs = predict_with_optimal_threshold(model, val_loader, optimal_threshold)
        oof_predictions[val_idx] = val_preds
        oof_probabilities[val_idx] = val_probs
        
        # 详细评估验证集
        fold_detailed_metrics = evaluate_detailed_metrics(
            y_fold_val, val_preds, f"Fold {fold_idx+1} 验证集(阈值={optimal_threshold:.3f})"
        )
        ...
        test_loader = DataLoader(test_dataset, batch_size=64, shuffle=False)
        test_preds, test_probs = predict_with_optimal_threshold(model, test_loader, optimal_threshold)
        ...
        print(f"Fold {fold_idx+1} 最佳: Bad_F1(优化后)={best_metrics['optimal_bad_f1']:.4f}, "
              f"阈值={optimal_threshold:.3f}, 训练轮数: {len(epoch_metrics)}")
    
    # 4. 整体OOF评估
    print(f"\n" + "="*60)
    print("📊 整体Out-of-Fold(OOF)评估结果 (使用各fold最优阈值)")
    print("="*60)
    
    oof_detailed_metrics = evaluate_detailed_metrics(
        y_train, oof_predictions, "OOF整体(阈值调优)"
    )
    
    # 5. CV统计分析
    cv_metrics = {
        'optimal_bad_f1': [r['best_metrics']['optimal_bad_f1'] for r in fold_results],
        'optimal_threshold': [r['optimal_threshold'] for r in fold_results],
        'bad_f1_default': [r['best_metrics']['bad_f1'] for r in fold_results],
        'f1_macro': [r['best_metrics']['f1_macro'] for r in fold_results],
        'f1_weighted': [r['best_metrics']['f1_weighted'] for r in fold_results],
        'accuracy': [r['best_metrics']['accuracy'] for r in fold_results],
        'bad_precision': [r['best_metrics']['bad_precision'] for r in fold_results],
    ...
    for metric_name, values in cv_metrics.items():
        mean_val = np.mean(values)
        std_val = np.std(values)
        marker = "🎯" if metric_name == 'optimal_bad_f1' else "  "
    ...
    # 阈值分析
    print(f"\n⚙️ 阈值调优分析:")
    print(f"   平均最优阈值: {np.mean(optimal_thresholds):.3f} ± {np.std(optimal_thresholds):.3f}")
    print(f"   阈值范围: {min(optimal_thresholds):.3f} - {max(optimal_thresholds):.3f}")
    print(f"   Bad F1提升: {np.mean(cv_metrics['optimal_bad_f1']) - np.mean(cv_metrics['bad_f1_default']):.4f}")
    ...
    print("=" * 80)
    print(f"📊 最佳配置: {best_config['name']}")
    print(f"🎯 CV Bad客户F1(优化后): {np.mean(cv_metrics['optimal_bad_f1']):.4f} ± {np.std(cv_metrics['optimal_bad_f1']):.4f}")
    print(f"🔄 CV Bad客户F1(默认0.5): {np.mean(cv_metrics['bad_f1_default']):.4f} ± {np.std(cv_metrics['bad_f1_default']):.4f}")
    print(f"⚙️ 平均最优阈值: {np.mean(optimal_thresholds):.3f}")
    print(f"📈 F1提升幅度: {np.mean(cv_metrics['optimal_bad_f1']) - np.mean(cv_metrics['bad_f1_default']):.4f}")
    print(f"🎯 OOF Bad客户F1: {oof_detailed_metrics['bad_f1']:.4f}")
    print(f"🔍 OOF准确率: {oof_detailed_metrics['accuracy']:.4f}")
    ...
    return {
        ...
        'improvement': np.mean(cv_metrics['optimal_bad_f1']) - np.mean(cv_metrics['bad_f1_default'])
    }

if __name__ == "__main__":
    results = main_best_config_cv_with_threshold()