...
def train_model(model, train_loader, val_loader, config, class_weights, epochs=100):
    ...
    for epoch in range(epochs):
        ...
        # 存储本轮指标 - 调整顺序，宏平均F1作为主要指标
        epoch_metrics.append({
            'epoch': epoch + 1,
            'train_loss': train_loss / len(train_loader),
            'f1_macro': val_f1_macro,  # 主要指标
        ...
        # 每10轮或最后几轮打印详细指标 - 突出显示宏平均F1
        if epoch % 10 == 0 or epoch >= epochs - 3:
            print(f'Epoch [{epoch+1:3d}]: mF1={val_f1_macro:.4f}, F1_w={val_f1_weighted:.4f}, Acc={val_accuracy:.4f}')
        ...
        if early_stopping(val_f1_macro, model):  # 使用宏平均F1进行早停
...
def evaluate_detailed_metrics(y_true, y_pred, title=""):
    ...
    for i, name in enumerate(class_names):
        if i < len(precision_per_class):
            print(f"   {name}: Precision={precision_per_class[i]:.4f}, "
...
def main_best_config_cv():
    """主函数 - 只使用最佳配置进行CV分析，使用宏平均F1"""
    
    print("=" * 80)
    print("🎯 MLP最佳配置CV预测分析 - 宏平均F1(mF1)指标版")
    print("=" * 80)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X_train_scaled, y_train)):
        ...
        # 获取最佳轮次的指标 - 使用宏平均F1
        best_epoch_idx = np.argmax([m['f1_macro'] for m in epoch_metrics])
        ...
        print(f"Fold {fold_idx+1} 最佳: mF1={best_metrics['f1_macro']:.4f}, "
              f"F1_w={best_metrics['f1_weighted']:.4f}, Acc={best_metrics['accuracy']:.4f}, "
              f"训练轮数: {len(epoch_metrics)}")
    
    # 4. 整体OOF评估
    print(f"\n" + "="*60)
    print("📊 整体Out-of-Fold(OOF)评估结果 (主要关注宏平均F1)")
    print("="*60)
    ...
    # 5. CV统计分析 - 将宏平均F1作为第一指标
    cv_metrics = {
        'f1_macro': [r['best_metrics']['f1_macro'] for r in fold_results],  # 主要指标放在第一位
        ...
        'bad_f1': [r['best_metrics']['bad_f1'] for r in fold_results]
    }
    
    print(f"\n📈 10折交叉验证统计 (主要指标: 宏平均F1):")
    for metric_name, values in cv_metrics.items():
        mean_val = np.mean(values)
        std_val = np.std(values)
        marker = "⭐" if metric_name == 'f1_macro' else "  "
    ...
    training_history_filename = f"MLP_training_history_{timestamp}.csv"
